...
## 3. 규제 기법  
현대 기계 학습은 아주 다양한 규제 기법을 사용합니다. 조기 멈춤 기법, L1, L2 규제, 맥스-놈, 드롭아웃, 앙상블 기법 등 다양한 규제 기법이 있습니다.  
...
드롭아웃이란, 입력층과 은닉층의 노드 중 일정 비율에 해당하는 노드를 임의로 제거하는 것입니다. 해당되는 노드의 들어오고 나가는 엣지들을 모두 제거합니다.(0을 출력합니다) 보통 드롭아웃될 확률의 0.1~0.5로 합니다.  
...
테스트시에는 드롭아웃을 사용하지 않는 보통 신경망처럼 전방 계산을 수행하는데 이 때 주의할 점은 가중치에 1-p(보존확률)만큼 (p는 드롭아웃 비율) 곱해줌으로써 가중치의 규모를 줄인 다음에 전방 계산을 수행합니다. 그 이유는 학습 과정에서 가중치는 1-p 만큼만 참여했기 때문입니다. 그렇지 않으면 각 노드가 훈련한 것보다 더 많은 입력 신호를 받기 때문에 잘 동작하지 않을 것입니다. 조금 더 일반적으로 말하면 훈련이 끝난 뒤 각 입력의 연결 가중치에 보존 확률를 곱해야 합니다.  
